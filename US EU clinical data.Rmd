...
eu2 = eu1 %>% select("eudract_number_with_country", "us_nct_clinicaltrials_gov_registry_number", "sponsors", "trial_term", "trial_therapeutic_area", "date_on_which_this_record_was_first_entered_in_the_eudract_data", "member_state_concerned", "trial_human_pharmacology_phase_i", "trial_therapeutic_exploratory_phase_ii", "trial_therapeutic_confirmatory_phase_iii", "trial_therapeutic_use_phase_iv", "subject_in_the_whole_clinical_trial", "trial_status", "date_of_the_global_end_of_the_trial", "trial_randomised", "trial_parallel_group", "trial_cross_over", "trial_controlled")
```
...
us_designs_reduced = us_designs %>% 
  ...
  select(nct_id, randomized, single_group)
...
us_datasets = list(us_studies_reduced, us_conditions_reduced, us_countries_reduced, us_sponsor_reduced, us_collaborator_reduced, us_designs_reduced)
...
eu7 = eu6 %>% 
  ...
  select("nct_id", "eudract_id", "sponsor", "industry_dummy", "country", "disease_code_eu", "start_year", "phase_1", "phase_2", "phase_3", "phase_4", "patient_num", "end_date", "status", "randomized", "single_group")
  
us3 = us2 %>% 
  rename(disease_code_us = disease_code) %>% 
  mutate(patient_num = as.numeric(patient_num)) %>% 
  select("nct_id", "sponsor", "industry_dummy", "country", "disease_code_us", "start_year", "phase_1", "phase_2", "phase_3", "phase_4", "patient_num", "end_date", "status", "randomized", "single_group")
...
```
 ...
 Now we want to apply fuzzy match on the combined list of company names from clinical_trials_4 and firm_founding_year2, in order to maximize the results, we want to separate the company names in the following groups:
  ...
  - Companies with names starting with a-g
  - Companies with names starting with h-q
  - Companies with names starting with r-z
...
firm_name_founding_year_id_numeric_start = firm_name_founding_year_id %>%
  filter(str_detect(sponsor_name, "^\\d")) %>% view()
...
firm_name_founding_year_id_a_to_g = firm_name_founding_year_id %>%
  filter(str_detect(sponsor_name, "^[A-Ga-g]")) %>% 
  mutate(sponsor_id = assign_fuzzy_index(sponsor_name, batch_size = 1000, cut_height = 0.1)) 
firm_name_founding_year_id_a_to_g$sponsor_id %>% max() # max sponsor_id 31635

firm_name_founding_year_id_h_to_q = firm_name_founding_year_id %>% 
  filter(str_detect(sponsor_name, "^[H-Qh-q]")) %>% 
  mutate(sponsor_id = assign_fuzzy_index(sponsor_name, batch_size = 1000, cut_height = 0.1)) 
firm_name_founding_year_id_h_to_q$sponsor_id %>% max() # max sponsor_id 31596

firm_name_founding_year_id_r_to_z = firm_name_founding_year_id %>%
  filter(str_detect(sponsor_name, "^[R-Zr-z]")) %>% 
  mutate(sponsor_id = assign_fuzzy_index(sponsor_name, batch_size = 1000, cut_height = 0.1)) 
firm_name_founding_year_id_r_to_z$sponsor_id %>% max() # max sponsor_id 22192
...
firm_name_founding_year_id = firm_name_founding_year_id_numeric_start %>% 
  rbind(firm_name_founding_year_id_a_to_g %>% mutate(sponsor_id = 276 + sponsor_id)) %>% 
  rbind(firm_name_founding_year_id_h_to_q %>% mutate(sponsor_id = 276 + 31635 + sponsor_id)) %>% 
  rbind(firm_name_founding_year_id_r_to_z %>% mutate(sponsor_id = 276 + 31635 + 31596 + sponsor_id)) %>% # 96222 observations
  distinct(sponsor_name, sponsor_id, year_founded) # get rid of duplicates, 96112 observations


# We know that if companies have different start_year values, they are for sure different sponsors, and for these sponsors that have the same sponsor_id, we call these false_duplicates and we want to assign new sponsor_id to them
false_duplicates = firm_name_founding_year_id %>% 
  filter(!is.na(year_founded)) %>% 
  group_by(sponsor_id) %>% 
  mutate(duplicate_flag = n_distinct(year_founded) > 1 | n_distinct(sponsor_name) > 1) %>% 
  filter(duplicate_flag == TRUE) %>% 
  select(-duplicate_flag) %>% 
  ungroup() # 7104 companies
  # Note: some company having different suffixs are different but are indistinguishable after cleaning sponsor names, for these copanies we can come back and distinguish them further if this highly effects the result. For now we choose the distinct sponsor names and treat them as the same company, thus getting rid of these "duplicates"
false_duplicates_clean = false_duplicates %>% 
  distinct(sponsor_name, .keep_all = TRUE) # 6844 companies
  # These are all the false duplicates of sponsor_ids, with their id relabeled to be distinct


# Then, we want to get rid of the false duplicates and add them back with new distinct sponsor_ids
firm_name_founding_year_id = anti_join(firm_name_founding_year_id, false_duplicates) # 96112 - 7104 = 89008 observations
firm_name_founding_year_id$sponsor_id %>% max() # max sponsor_id 85699
firm_name_founding_year_id = firm_name_founding_year_id %>% 
  rbind(false_duplicates_clean %>% mutate(sponsor_id = 85699 + row_number())) # 89008 + 6844 = 95852 observations
...
clinical_trials_5 = clinical_trials_4 %>% 
  ...
  select("nct_id", "eudract_id", "sponsor_name", "industry_dummy", "sponsor_id", "disease_code", "disease_code_eu", "disease_code_us", "disease_term", "disease_id", "start_year",  "country", "phase_1", "phase_2", "phase_3", "phase_4", "patient_num", "end_date", "status", "randomized", "single_group")
...
clinical_trials_6 = clinical_trials_5 %>% 
  ...
  select("nct_id", "eudract_id", "sponsor_name", "industry_dummy", "sponsor_id", "disease_code", "disease_code_eu", "disease_code_us", "disease_term", "disease_id", "start_year",  "country", "country_id", "phase_1", "phase_2", "phase_3", "phase_4", "patient_num", "end_date", "status", "randomized", "single_group")
...
# From 865051 to 863041 observations
# We reduced 2010 duplicates
```
...
# From 863041 to 859411
# We reduced 3630 duplicates
```
...
main_raw_dataset = main_raw_dataset %>% 
  select(trial_id, names(main_raw_dataset)) # 859411
...
trial_country_dataset = main_raw_dataset %>% 
  select(trial_id, nct_id, eudract_id, country, country_id) # 859411

trial_disease_firm_dataset = main_raw_dataset %>% 
  select(trial_id, nct_id, eudract_id, sponsor_name, industry_dummy, sponsor_id, disease_code, disease_code_eu, disease_code_us, disease_term, disease_id, start_year, phase_1, phase_2, phase_3, phase_4, patient_num, end_year, status) # 859411
...
trial_firm_detail_1 = main_raw_dataset %>% 
  ...
  select(trial_id, sponsor_name, sponsor_id, sponsor_found_year, start_year, country, phase_1, phase_2, phase_3, phase_4, patient_num, status, end_year, industry_dummy, source, disease_id, nct_id, eudract_id, randomized, single_group) %>% 
...
trial_firm_detail_2 = trial_firm_detail_1 %>% # 859441
  filter(sponsor_name != "") %>% # 859177
  filter(!(phase_1 == 0 & phase_2 == 0 & phase_3 == 0 & phase_4 == 0)) %>% # 507555, big cut
  filter(start_year != "") %>% # 505009
  filter(start_year > sponsor_found_year) %>% # 501800
  mutate(end_year = ifelse(end_year > "2024", "", end_year)) %>% 
  filter(!(end_year == "" & (status == "completed" | status == "terminated"))) %>% # 466148, big cut
  mutate(status = ifelse(status == "" & end_year == "", "ongoing", status)) %>% 
  filter(!(status == "" & !(end_year == ""))) %>% # 445731
  mutate(end_year = ifelse(end_year == "", "3000", end_year)) %>% 
  filter(start_year <= end_year) %>% # 445137
  mutate(end_year = ifelse(end_year == "3000", "", end_year))
...
##### Add detailed variables for phase_2_start_eu
...
##### Add variable phase_2_active, phase_2_comp_rate and phase_2_term_rate 
...
phase_2_active_comp_term_rate = firm_year_dataset_6 %>%
  select(sponsor_id, year, phase_2_start, phase_2_enddate_comp, phase_2_enddate_term) %>% 
  ...
  select(sponsor_id, year, phase_2_active, phase_2_comp_rate, phase_2_term_rate) 


firm_year_dataset_7 = firm_year_dataset_6 %>% 
  left_join(phase_2_active_comp_term_rate, by = c("sponsor_id", "year")) %>% 
  mutate(across(everything(), ~ replace_na(., 0)))

# 379114, due to many-to-many left join, investigate this
```
...
# 379114, same as vol.7 but more than vol.6 becasue of many-to-many joining
```

##### Add (startdate_random_rate, startdate_single_group_rate, startdate_parrell_group_rate) and (enddate_random_rate, and enddate_single_group_rate, enddate_parrell_group_rate)

  We now want to discover other attributes of clinical trials, particularly in if a trial is randomized or not, and if a trial's intervention type is single-group assignment or not. So we want to create randomized_rate and single_group_rate accordingly.
  
```{r}
# Create randomize_single_group containing information of randomized and single_group
randomized_single_group = trial_firm_detail_3 %>% 
  ...
  select(sponsor_id, start_year, end_year, randomized, single_group)


# Create variable startdate_random_rate, startdate_single_group_rate
startdate_random_single_group_rate = randomized_single_group %>% 
  group_by(sponsor_id, start_year) %>% 
  summarize(startdate_random_rate = mean(randomized),
            startdate_single_group_rate = mean(single_group))

# Create variable enddate_random_rate, enddate_single_group_rate
enddate_random_single_group_rate = randomized_single_group %>% 
  group_by(sponsor_id, end_year) %>% 
  summarize(enddate_random_rate = mean(randomized),
            enddate_single_group_rate = mean(single_group))

# Add these 4 columns to the firm_year_dataset
firm_year_dataset_9 = firm_year_dataset_8 %>% 
  left_join(startdate_random_single_group_rate, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(enddate_random_single_group_rate, by = c("sponsor_id" = "sponsor_id", "year" = "end_year")) %>% 
  mutate(across(everything(), ~ replace_na(., 0))) 
...
```{r}
# Exploratory code
firm_year_dataset_9 %>% filter(str_detect(sponsor_name, "novartis"))
firm_name_founding_year_id %>% filter(str_detect(sponsor_name, "novartis"))
...
write.csv(firm_year_dataset_9, file = "firm_year_dataset_11072024.csv")
```
