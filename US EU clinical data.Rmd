...
Before we complete the construction of the datsset, which includes a lot of interation between variables, we want to set definitions of what defines a "valid" observation, here;s how I can think of to define a "valid" trial:
 ...
 - A trial having an end year but not a status will be *exclude* (considered completed or terminated?)
 
Additionally, for companies missing sponsor_found_year, we will use their earliest trial start year as their founded year
...
trial_firm_detail_3 = trial_firm_detail_2 %>% # 859441
  ...
  filter(!(end_year == "" & (status == "completed" | status == "terminated"))) %>% # 339522, big cut
  mutate(status = ifelse(status == "" & end_year == "", "ongoing", ifelse(status == "", "completed", status)))
...
```
  ...
  Create 12 columns tracking number of phase 1 to 4 trials starting, completing, or terminating throughout the years

```{r}
# phase 1 start, complete, terminate
phase_1_start = trial_firm_detail_3 %>% 
  filter(phase_1 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_1_start = n(), .groups = 'drop') 

phase_1_complete = trial_firm_detail_3 %>% 
  filter(status == "completed") %>% 
  filter(phase_1 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_1_complete = n(), .groups = 'drop') 

phase_1_terminate = trial_firm_detail_3 %>% 
  filter(status == "terminated") %>% 
  filter(phase_1 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_1_terminate = n(), .groups = 'drop') 


# phase 2 start, complete, terminate
phase_2_start = trial_firm_detail_3 %>% 
  filter(phase_2 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_2_start = n(), .groups = 'drop') 

phase_2_complete = trial_firm_detail_3 %>% 
  filter(status == "completed") %>% 
  filter(phase_2 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_2_complete = n(), .groups = 'drop') 

phase_2_terminate = trial_firm_detail_3 %>% 
  filter(status == "terminated") %>% 
  filter(phase_2 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_2_terminate = n(), .groups = 'drop') 


# phase 3 start, complete, terminate
phase_3_start = trial_firm_detail_3 %>% 
  filter(phase_3 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_3_start = n(), .groups = 'drop') 

phase_3_complete = trial_firm_detail_3 %>% 
  filter(status == "completed") %>% 
  filter(phase_3 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_3_complete = n(), .groups = 'drop') 

phase_3_terminate = trial_firm_detail_3 %>% 
  filter(status == "terminated") %>% 
  filter(phase_3 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_3_terminate = n(), .groups = 'drop') 


# phase 4 start, complete, terminate
phase_4_start = trial_firm_detail_3 %>% 
  filter(phase_4 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_4_start_ = n(), .groups = 'drop') 

phase_4_complete = trial_firm_detail_3 %>% 
  filter(status == "completed") %>% 
  filter(phase_4 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_4_complete = n(), .groups = 'drop') 

phase_4_terminate = trial_firm_detail_3 %>% 
  filter(status == "terminated") %>% 
  filter(phase_4 == 1) %>% 
  group_by(sponsor_id, start_year) %>% 
  summarise(phase_4_terminate = n(), .groups = 'drop') 

firm_year_dataset_2 = firm_year_dataset_1 %>% 
  left_join(phase_1_start, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_1_complete, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_1_terminate, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_2_start, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_2_complete, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_2_terminate, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_3_start, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_3_complete, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_3_terminate, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_4_start, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>%
  left_join(phase_4_complete, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
  left_join(phase_4_terminate, by = c("sponsor_id" = "sponsor_id", "year" = "start_year")) %>% 
...
eu_expose_pre_2016 = trial_firm_detail_3 %>% 
  ...
  group_by(sponsor_id) %>% 
  summarise(eu_porp_pre_2016 = mean(any_eu_pre_2016), # calculate proportion of eu trials
            any_eu_pre_2016 = max(any_eu_pre_2016)) # 1 if any trial is in EU) 


eu_prop_median = median(eu_expose_pre_2016$eu_porp_pre_2016) # Calculate the median of EU proportion

eu_expose_pre_2016 = eu_expose_pre_2016 %>% 
  mutate(above_median_eu_pre_2016 = ifelse(eu_porp_pre_2016>eu_prop_median, 1, 0)) %>% # 1 if EU proportion > median
  select(-eu_porp_pre_2016)

firm_year_dataset_3 = firm_year_dataset_2 %>% 
  left_join(eu_expose_pre_2016, by = "sponsor_id") %>% 
  mutate(any_eu_pre_2016 = ifelse(is.na(any_eu_pre_2016), 0, 1)) %>%  # Notice companies founded after 2016 wil have NA values
  mutate(above_median_eu_pre_2016 = ifelse(is.na(above_median_eu_pre_2016), 0, 1))




# Use this code to obtain exposure on company level, independent of time
firm_year_dataset_3 %>% distinct(sponsor_id, any_eu_pre_2016, above_median_eu_pre_2016, .keep_all = TRUE) %>% 
  select(sponsor_id, sponsor_name, any_eu_pre_2016, above_median_eu_pre_2016)

firm_year_dataset_3 %>% filter(sponsor_id == "21373")
```
...
write.csv(trial_firm_detail_2, file = "trial_firm_detail_2.csv")
write.csv(unmatched_companies_with_industry_dummy, file = "unmatched_companies_with_industry_dummy.csv")
...
# Task
 ...
 2. maybe discard companies with on ly one trial
 3. fix filtering in trial_firm_detail
 4. add industry_dummy for firm_year_data, maybe drop academic in the future
 5. send csv of unmatched_dummy with industry dummy variable, and firm_year_data
 6. For the firm/year panel, let's also include # of us-based trials and # of EU-based trials (per each phase) per firm/year.  Basically, this will be # of phase I, 2, 3 trials but based on where the country for that trial is (EU or US). If neither US or EU, maybe we can create separate counts too.
